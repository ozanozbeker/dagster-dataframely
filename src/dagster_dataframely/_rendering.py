...
A schema's constraints reach a data consumer as three kinds of text: a constraint, a name, and a description. Each surface falls back differently.

| surface | fallback |
| --- | --- |
| column constraint | rendered constraint, then the rule's own name. Docstring excluded. |
| check name | always `dy_rule__<rule>`. Identity, never prose. |
| check description | docstring, then `<column> <rendered>`, then the rule name. |
| collapsed check description | one column's members: every rendered constraint, then that member's own kind. Docstring excluded, for the same reason a column constraint excludes it. The members no column owns are listed by name instead, because a `@dy.rule()` has no constraint to render and a rendered `primary_key` would put its own commas inside a comma-separated list. |

The register rule keeps the docstring out of a constraint. Two sibling rules must not read in different voices merely because one author wrote a docstring and the other did not. From the UI that is a change of register with no visible cause. So the rule name is the constant on the constraint surfaces, and the docstring reaches only the description.

Values are rendered but never named. A bound is a parameter, so it belongs in a constraint and in the per-evaluation `dy_rule__expr` metadata. It never belongs in a check name that renaming would orphan.
...
def _value(column: dy.Column, kind: str) -> Any:  # noqa: ANN401 - the values are of every constraint's own type
    ...
    By name rather than by attribute access, because these values live on Dataframely mixins that are private (`OrdinalMixin`, `IsInMixin`). Importing them to satisfy a type checker would take a dependency on private structure for no runtime gain. A characterization test covers the regularity they provide instead.
    ...
    kind
        The rule's own half of its name, which is also the parameter's.

    Returns
    -------
    Whatever the author passed that parameter, at whatever type they passed it: a number for a bound, a string for a regex, a sequence for `is_in`. Every caller renders it rather than reasoning about it, so one accessor serves all of them.
    """
...
def _length_unit(column: dy.Column) -> str:
    ...
    They are the only two column types that take the parameters at all. A characterization test covers that (#16) rather than this function assuming it.
    """
...
def _named_check(kind: str) -> str:
    ...
    A `check={"lowercase": ...}` reaches the reader as `lowercase`. That is the nudge to name a check rather than passing a bare lambda.
    """
...
def _column_constraint(column: dy.Column, kind: str) -> str | None:  # noqa: PLR0911 - one return per constraint shape; a dispatch table of lambdas would be the same table read through one more indirection
    ...
    kind
        The rule's own half of its name, so `min` for `amount|min`.
...
def table_constraints(schema: type[dy.Schema]) -> list[str]:
    ...
    The primary key above all. Dataframely models it as one rule over a struct of every key column, so stating it once, here, distinguishes a composite key from two independent single-column ones. A `@dy.rule()` joins it under its own name, because its expression is arbitrary and its docstring belongs to the description.
...
def column_rule_summary(schema: type[dy.Schema], rule_names: Sequence[str]) -> str:
    ...
    A collapsed check's description is the only place its members are visible before a run, so it renders all of them, including the two a constraint skips. `not null` has to be said here, because no column row sits beside this check saying it instead.

    The check names the column once, so no constraint repeats it. A rule with nothing structured to render falls back to its own half of its name.
