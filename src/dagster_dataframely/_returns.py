...
A decorated function returns the frame to validate, or a `dg.MaterializeResult` carrying it. Dagster's own docs teach the second spelling for attaching metadata, and it is the only route to a materialization's tags and data version. Refusing it would cost parity with the `@dg.asset` this decorator is modelled on (#77).
...
def unwrap(
    ...
    A bare frame passes straight through with nothing to fold. Returning one is therefore byte-for-byte what it was: no metadata, tags or data version can appear on that path that did not appear before.
    ...
    ----------
    returned
        Whatever the decorated function returned, frame or result alike.
    asset
        The valid out's asset key, rendered, for the messages.

    Returns
    -------
    The frame `process` validates, and the result to fold over its yields. The result is `None` when a bare frame was returned.
    ...
    MaterializeResultValueError
        The result carries no frame on `value`.
    """
