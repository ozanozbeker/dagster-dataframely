...
A data consumer gets a `skimr`-style distribution off the asset itself rather than reaching for a separate tool. Every materialization carries them unless the `statistics` setting turns the pass off.
...
**The string family carries no value-bearing statistic, deliberately.** A `min` or `max` on an email column would print real addresses permanently into a shared, exported event log. Lengths and cardinality catch what you would actually catch. The setting does not cover this: consenting to summary statistics is not consenting to raw values.
...
Its own module rather than `_metadata`'s. That module holds what an asset declares about its data before it has ever run. This one is computed from what a run actually wrote, and the two answer to different things when the schema and the frame disagree.
"""
...
#: The dtypes whose useful statistics are all lengths and counts. Sharing one set of statistics is what makes them one table rather than four.
_STRING_FAMILY = (pl.String, pl.Categorical, pl.Enum, pl.Binary)


def _cell(value: object) -> str | int | float | bool | None:
    """Render one aggregate as something a table record accepts.
...
def _rounded(value: object) -> str | int | float | bool | None:
    """Render a statistic that was computed rather than observed, rounding what `_cell` returns."""
    cell = _cell(value)
...
def _record(column: str, stats: Mapping[str, object]) -> dg.TableRecord:
    """Render one column's aggregates as the row a table shows.
    ...
    ----------
    column
        The column the row is about, which is the table's first cell.
    stats
...
def _temporal(name: str, dtype: pl.DataType) -> pl.Expr:
    ...
    The span is a duration whatever the column is. Subtracting two dates, two times or two durations gives one. Polars' friendly form renders it, because nobody reads a range off ISO-8601. A `Duration` column's own bounds are rendered the same way, since they are the same kind of value as the span between them.

    The other three dtypes keep their values through the aggregate and become strings at the display step. That is what makes a `Date` render as a date.
    """
    column = pl.col(name)
...
def _boolean(name: str, _dtype: pl.DataType) -> pl.Expr:
    ...
    Both counts are stated rather than one and a total, because those two plus `null_count` add up to `count` and a reader should not have to do that subtraction. The rate is over the non-null rows, which is what `mean` gives.
    """
    column = pl.col(name)
    return pl.struct(
        count=column.len(),
...
def _family(dtype: pl.DataType) -> str | None:
    ...
    Polars' own predicates rather than a transcribed dtype list, so a new integer width reaches the numeric family without this file being touched.
...
def statistics_metadata(
    ...
    enabled
        Whether to compute them at all. `False` returns before the frame is touched, so a setting turned off costs nothing rather than costing a pass whose result is discarded.
