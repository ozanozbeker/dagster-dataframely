...
| # | Claim | Verdict |
| - | ----- | ------- |
| 1 | `dagster/column_schema` lives in three buckets, definition, output type and materialization, each rendered in a different UI surface | **CONFIRMED** (one sub-claim corrected: the buckets *do* clash on the Catalog Overview) |
...
Reproduced: an asset with `@dg.asset(metadata={"defn_only_key": ...})` produced a materialization event whose metadata keys were only what the IO manager emitted.
`defn_only_key` was absent.
...
That removes IO-manager subclassing as a *forced* dependency of the UI story.
It may still be wanted for other reasons (casting on store, quarantine paths), but the metadata argument no longer carries it.
...
- `DagsterTypeSummary` selects the entry by `gqlTypePredicate('TableSchemaMetadataEntry')`, so **any** `TableSchema`-typed entry matches, regardless of key.
...
It is the only in-body mechanism that beats a third-party IO manager, and it is partition-aware.
That is directly relevant to the map's open "Partitioned assets" fog, since a per-partition schema or row count can be attached without touching the IO manager.
...
- `AssetSpec.merge_attributes(...)` / `AssetSpec.replace_attributes(...)` and the module-level `dg.map_asset_specs(...)` give a clean, non-invasive way to *decorate existing assets* with a schema.
  This is a much lighter entry point than the archive's `@dd.asset` `__annotations__` rewriting: a `dagster_dataframely` helper could be a `map_asset_specs` pass over anyone's assets.
