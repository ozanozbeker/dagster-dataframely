...
- **[docs]**: the published documentation claims it, and it is not independently verified (and in one case, verified *false*).
...
- `dagster-pandera` (2022) renders it into a structured **`TableSchema`** under `DagsterType.metadata["schema"]`.
  Higher fidelity, but under a *private* metadata key on the `DagsterType` rather than the ecosystem-standard `dagster/column_schema` key on the asset.
...
**[executed]** A `DagsterType` type check runs **before** the IO manager, and failure raises `DagsterTypeCheckDidNotPass`.
No output is stored, no materialization event is emitted, and there is no severity dial.
...
Source: [`dagster_pandera/__init__.py:323-325`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera/__init__.py#L323-L325) **[source]**
...
Source: [`__init__.py:145-153`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera/__init__.py#L145-L153) **[source]**
...
Source: [`dagster_pandera_tests/test_polars.py:120-160`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera_tests/test_polars.py) **[source]**
...
Source: [`__init__.py:292-320`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera/__init__.py#L292-L320) **[source]**
...
Source: [`__init__.py:247-253`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera/__init__.py#L247-L253) **[source]**
...
Source: [`__init__.py:116-119`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/dagster_pandera/__init__.py#L116-L119), reproduced on <https://docs.dagster.io/integrations/libraries/pandera/dagster-pandera> **[docs]**
...
Source: [`dagster-pandera/pyproject.toml`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandera/pyproject.toml) **[source]**
...
Source: [`data_frame.py:106-111`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandas/dagster_pandas/data_frame.py#L106-L111) **[source]**

No `TableSchema` is derived from the declared schema.
The package *does* ship `create_table_schema_metadata_from_dataframe`, but it reflects the schema off a **runtime dataframe**, and it is not wired into either factory, so the user must call it themselves from `metadata_fn`; [`data_frame.py:114-135`](https://github.com/dagster-io/dagster/blob/master/python_modules/libraries/dagster-pandas/dagster_pandas/data_frame.py#L114-L135).
**[source]**
...
1. **Do not make `DagsterType` the primary attachment mechanism.**
   ...
   Dataframely's whole value proposition, `filter()` returning valid rows *plus* a persistable `FailureInfo`, cannot be expressed on a substrate that discards the output on failure.
...
9. **Do not read either package's docs as a spec.**
   ...
   Both packages are `@beta`, feature-frozen, and unowned.
   The `dagster-polars` maintainer states outright on #23000 that they do not maintain `dagster-pandera`.
...
- Granularity of generated asset checks (one per schema, one per column, one per rule).
  No prior art in this survey generates asset checks at all, so there is nothing to copy here.
  This is greenfield.
