...
**A test reflects the set off the package rather than listing it.** It finds every public callable in `dagster_dataframely` and `dagster_dataframely.wiring` whose first parameter is annotated `type[dy.Schema]`, and calls each with `None` for every other argument. Nothing else is touched when the guard runs first, so `ReservedColumnError` proves the ordering and a `TypeError` proves the guard sits too late. A ninth such function is covered the day it is written.

Stubbing the other arguments only works while the schema comes first, so the test asserts that convention rather than assuming it. A function taking a schema second would otherwise contribute no cases and leave the suite green, which is the one way this guarantee could lapse unnoticed.

This does not repeat the objection `test_public_surface.py` raises against deriving expectations from `__all__`. A reflected set generates assertions rather than relaxing them: a new name adds a test that can fail, and can never remove one. That file keeps the separate question of whether the name should be public at all, so the reflection asserts it found at least the eight known here.
