...
So a unit test had to go through `dg.materialize`, standing up an IO manager and an instance for what should be a function call. That cost was paid by every user writing a test for their own decorated function, not only by this package's suite.
...
One thing was never broken: a decorated function may declare `context`, bare or annotated, alone or alongside upstream frame inputs. `functools.wraps` puts its signature in front of Dagster, so the parameter binds as it does anywhere else. It was untested, and the documentation denied it.
...
Neither change touches `process`'s signature. ADR-0001's decision stands, and it is what made this possible. A `process` still taking a context would have kept blocker 1 for hand-wiring and for the decorator alike.
...
A decorated function declaring `context` is invoked as `orders(dg.build_asset_context(partition_key="2026-01-02"), raw_orders)`. A shape drift raises `SchemaShapeError` out of the call rather than wrapped in a step failure.
...
Nothing a run sees changes. The same asset keys, check names, severities and metadata, and the same five exits. What moves is where the checks sit in the event stream. A bundled result is emitted with its materialization, and a standalone one after it.
...
**An opt-in `context` parameter, detected on the decorated function and passed through.** The obvious reading of the original report. Rejected because it closes nothing: blocker 2 reproduces on a bare `@dg.multi_asset` taking a `context`, so the wrapper's own `asset_key_for_output` call would still fail on a direct call. It also buys nothing, because the parameter already works.
