...
The surface is spelled out here rather than derived from `__all__`. A test that reads its expectation off the thing it is testing agrees with every change. Adding a name to the package is meant to cost a line in this file, which is where the question "should a user see this?" gets asked.
...
def test_the_error_family_is_exported_whole():
    ...
    Derived from the base rather than listed, because the failure this guards against is a new error added to the package and forgotten in `errors.__all__`.
    """
...
def test_py_typed_ships_in_the_built_wheel(tmp_path: Path):
    """The marker makes the annotations visible to a type checker downstream. It is a file rather than code, so nothing but a build proves it made the trip.
