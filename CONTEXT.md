...
Attaches a Dataframely schema to a Dagster asset.
...
The terms below are the ones this package had to add, plus a few that were often used wrongly.
_Avoid_ lists words not to use **for the term above it**.
The rule covers the meaning, not the string: `sink_parquet` is Polars' own name and not a writer, so it is allowed.

This is a glossary.
The reason a thing works the way it does belongs in `docs/adr/` or in the module that does it.
...
Dagster's own word, because `dd.asset` builds a `dg.asset` with a schema attached, by calling `@dg.asset`.
...
Upstream assets are passed to it as parameters, it may declare `context`, and it returns the frame to validate, a `dg.MaterializeResult` whose `value` is that frame, or `None` to skip.
Dagster's own phrase.
...
Dataframely's own word, and the one its rule columns use.
...
Say what happened: no rows failed; rows failed and no quarantine is declared; no rows were valid; the decorated function returned `None`.
...
**Quarantine**: Where invalid rows are written, under the asset key `<name>_quarantine`.
...
Without it, any invalid row fails the run, and nothing is written.
...
When it fails, the run stops before any row is filtered.
...
**Granularity**: How many checks the rules are collapsed into, which `check_granularity` sets: one per rule at `rule`, one per column at `column`, one for the schema at `schema`.
...
Always name what is collapsed: the rules are collapsed into checks, and a check that reports for several of them is a collapsed check.
...
That is why the decorator is `dd.asset` and not `dy_asset`.
The quarantine address is the exception: the outcomes that raise have no materialization, so each check result has `dataframely/quarantine_address`, the same key the materialization uses (ADR-0004).
...
Say "form" or "syntax" for how a value or a name is written.
